  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex gap-1 rounded-lg bg-cantera-sand/40 p-1">
          {FILTROS.map((f) => (
            <button
              key={f.value}
              type="button"
              ...
              className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${
                filtro === f.value ? 'bg-white text-cantera-ink shadow-sm' : 'text-cantera-secondary'
              }`}
            >
              {f.label}
            </button>
        ...
        <select
          ...
          className="rounded-lg border border-cantera-sand px-3 py-2 text-sm focus:border-cantera-primary focus:outline-none"
        >
      ...
      {filtrados.length === 0 && (
        <p className="text-cantera-secondary">No encontramos insumos que coincidan con estos filtros.</p>
      )}
      ...
      <div className="flex flex-col gap-2">
        {filtrados.map((i) => {
          ...
          return (
            ...
            >
              <div>
                <p className="font-semibold text-cantera-ink">{i.nombre}</p>
