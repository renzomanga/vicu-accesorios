...
export default function LoginPage() {
  const [state, formAction, pending] = useActionState(enviarMagicLink, {
    ok: false,
    error: null,
  })
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-rose-50 px-6">
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-sm">
        ...
        <p className="mt-1 text-sm text-zinc-500">Ingresá con tu email, sin contraseña.</p>

        {state.ok ? (
          <p className="mt-6 rounded-lg bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            Te enviamos un link a tu email. Abrilo desde este mismo dispositivo para entrar.
          </p>
        ) : (
